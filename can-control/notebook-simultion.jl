...
# ╔═╡ 190a0cb0-f79d-11ed-2d6e-f74efe476424
using Plots, LinearAlgebra, ControlSystems

# ╔═╡ e045d1d2-be33-4cda-8308-a6dbca91c5c3
# Função de referência
#=function reference(t)
    return sin(2π*t)
end=#

reference = (t) -> sin(2π*t)

# ╔═╡ 304c0ca4-5916-415f-9e8a-276cc3825468
begin
	# Matrizes de estado
	A = [0.5 0; 0 0.2]
	B = [1, 0]
...
# ╔═╡ 6c15e120-8427-4e7f-9bfc-e4fdc6b1719e
# Projeto do controlador RST
function rst_control(sys, reference)
    # Obtenção dos polos desejados
    poles = [-0.5, -0.6]

    # Cálculo das matrizes de controle
    _, X, K = ctrb(A, B)
    Acl = A - B * K * C

    #Cálculo da matriz de transição
    T = zeros(size(Acl))
    for i = 0:length(poles)-1
        T[i+1] += exp(poles[i+1]) * (Acl^i) / factorial(i)
    end
    ...
    # Definição da função de transferência do controlador RST
    controlador = tf(ss(Acl, B, C, D))

    return controlador
end

# ╔═╡ 8323cfcc-6b2e-4d50-a6e5-35a27852d42b
begin
	sys = ss(A, B, C, D)
	controlador = rst_control(sys, reference)
end

# ╔═╡ 8bcff461-7b18-4d19-a878-e57c97cbbc4e
_, X, K = ctrb(A, B)

# ╔═╡ 768805c0-c08e-4992-bb43-06b2a6614f1d
begin
	T = K * B * C
...
# ╔═╡ 6c377246-041f-42a3-97b7-259fd970f269
let
	tf("s")
	Gs = tf([1], [10, 1])
	c2d(Gs, 1) |> zpkdata
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "cce37d4c12b40e0b4f3fc8d7a7adfd9369cf7216"
